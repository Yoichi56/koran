...
class beban extends CI_Controller {
	...
	public $tabel_db	=	"beban"; #	nama tabel di db
	public $pk 			= 	"kode_beban"; #	nama primary key di tabel ^^
	public function index() {
		$data['judul'] 		=	"Beban";
		$data['menu'] 		=	"Master Data";
		$data['icon'] 		=	"fa fa-object-group";
		$data['hasil'] 		=	$this->master_data->tampilkan($this->tabel_db);
		$data['url']		=	site_url('beban/tambah_beban');
		$this->load->view('template/header', $data);
		$this->template->load('template/content', 'master/beban/index', $data);
	...
	public function tambah_beban() {
		...
		if ($this->form_validation->run() == FALSE) {
			$this->load->view('template/header', $data);
			$this->template->load('template/content', 'master/beban/form', $data);
			$this->load->view('template/footer');	
		}
		else{
	...
	public function formUbahbeban() {
		$data['judul'] 		=	"Beban";
		$data['menu'] 		=	"Master Data";
		$data['icon'] 		=	"fa fa-object-group";
		$data['url']		=	site_url('beban/ubah_beban');
		$data['back']		=	site_url('beban');
		$data['hasil'] 		=	$this->master_data->ambil1Data($this->tabel_db, $this->pk, $this->uri->segment(3));
		$this->load->view('template/header', $data);
		$this->template->load('template/content', 'master/beban/edit', $data);
		$this->load->view('template/footer');	
	}
